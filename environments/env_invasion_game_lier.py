...
class TaskEnvironment(object):
	...
	def __init__(self):
		self.num_actions = 2
		self.num_colors = 2
		self.num_percepts_list = np.array([self.num_actions, self.num_colors])
		self.next_state = np.array(
			[np.random.randint(self.num_actions), np.random.randint(self.num_colors)])
	...
	def move(self, action):
		if (
			(action == self.next_state[0]) & (self.next_state[1] == 0) #Se o invasor não mentiu
			) | ((action != self.next_state[0]) & (self.next_state[1] == 1) #Se o invasor mentiu
		):
			reward = 1
		else:
			reward = 0
